...
function asqr_register_github_updater(): void
{
    if (!is_admin()) {
        return;
    }

    $updater = new ASQR_GitHub_Updater();
...
class ASQR_GitHub_Updater
{
    private const OWNER = 'cchatterton';
    private const REPO = 'as-qs-relay';
    private const SLUG = 'as-qs-relay';
    private const ASSET_NAME = 'as-qs-relay.zip';
    private const RELEASE_TRANSIENT = 'asqr_github_latest_release';
    private const ERROR_TRANSIENT = 'asqr_github_latest_release_error';
    ...
    public function add_update_data($transient)
    {
        ...
        $plugin_file = ASQR_PLUGIN_BASENAME;
        ...
        if ($version && $package && version_compare($version, ASQR_VERSION, '>')) {
    ...
    public function plugin_details($result, string $action, object $args)
    {
        ...
        return (object) array(
            'name' => 'AS QS Relay',
            'slug' => self::SLUG,
            'version' => $this->release_version($release) ?: ASQR_VERSION,
            'author' => 'AlphaSys',
            ...
            'requires_php' => '8.1',
            'sections' => array(
                'description' => __('Maintains a first-party JSON cookie of tracked query-string touchpoints over time.', 'as-qs-relay'),
    ...
    {
        if (ASQR_PLUGIN_BASENAME !== $file) {
            return $links;
        }

        $links[] = '<a href="' . esc_url($this->repo_url()) . '">' . esc_html__('GitHub', 'as-qs-relay') . '</a>';
        $links[] = '<a href="' . esc_url($this->check_updates_url()) . '">' . esc_html__('Check for updates', 'as-qs-relay') . '</a>';
        return $links;
    }

    public function clear_cache_after_upgrade($upgrader, array $hook_extra): void
    {
        if (($hook_extra['type'] ?? '') !== 'plugin' || empty($hook_extra['plugins']) || !in_array(ASQR_PLUGIN_BASENAME, (array) $hook_extra['plugins'], true)) {
            return;
        }
    ...
    {
        if (empty($_GET['asqr_check_updates'])) {
            return;
        }

        if (!current_user_can('update_plugins')) {
            wp_die(esc_html__('You do not have permission to check for plugin updates.', 'as-qs-relay'));
        }

        check_admin_referer('asqr_check_updates');
        delete_site_transient(self::RELEASE_TRANSIENT);
        delete_site_transient(self::ERROR_TRANSIENT);
        ...
        wp_safe_redirect(add_query_arg('asqr_checked_updates', '1', $this->plugins_page_url()));
        exit;
    }
    ...
    private function latest_release(): ?array
    {
        ...
        $response = wp_remote_get('https://api.github.com/repos/' . self::OWNER . '/' . self::REPO . '/releases/latest', array(
            ...
            'headers' => array(
                ...
                'User-Agent' => 'AS-QS-Relay/' . ASQR_VERSION,
            ),
        ));
        ...
        $cache_length = version_compare($this->release_version($release), ASQR_VERSION, '>') ? 6 * HOUR_IN_SECONDS : 5 * MINUTE_IN_SECONDS;
    ...
    {
        return wp_nonce_url(add_query_arg('asqr_check_updates', '1', $this->plugins_page_url()), 'asqr_check_updates');
    }
